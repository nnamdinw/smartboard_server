...
imu::Quaternion Adafruit_BNO055::getQuat() {
  ...
  /* Read quat data (8 unsigned chars) */
...
/*!
 *  @brief  Reads the sensor's offset registers into a unsigned char array
 *  @param  calibData
...
void Adafruit_BNO055::setSensorOffsets(const uint8_t *calibData) {
  ...
  /* Note: Configuration will take place only when user writes to the last
     unsigned char of each config data pair (ex. ACCEL_OFFSET_Z_MSB_ADDR, etc.).
     Therefore the last unsigned char must be written whenever the user wants to
 ...
 */
bool Adafruit_BNO055::readLen(adafruit_bno055_reg_t reg, unsigned char *buffer, uint8_t len) {
