...
MPU9250::MPU9250()
{
	setFd(wiringPiI2CSetup(MPU9250_ADDRESS));
}
...
void MPU9250::readAccelData(int16_t * destination)
{
  ...
  //readBytes(i2cAddress,i2cDeviceRegister,len,dst);
...
void MPU9250::writeByte(uint8_t address, uint8_t subAddress, uint8_t data)
{
  ...
  */
  ...
  wiringPiI2CWriteReg8(getFd(),subAddress,data);
}
...
uint8_t MPU9250::readByte(uint8_t address, uint8_t subAddress)
{
  ...
  */
  data = wiringPiI2CReadReg8(getFd(),subAddress);
...
{  
  ...
  {
    dest[i] = wiringPiI2CReadReg8(getFd(),subAddress);
  }
...
bool MPU9250::poll()
{
	bool result = false;
               if(readByte(MPU9250_ADDRESS,INT_STATUS) & 0X01)
                  {
                          readAccelData(accelCount); //read xyz adc values
                      	  getAres();
