...
MPL3115A2::MPL3115A2()
{
  //Set initial values for private var
  //default i2c address is 0x60
 setFd( wiringPiI2CSetup(MPL3115A2_ADDRESS));
...
//	if (Wire.requestFrom(MPL3115A2_ADDRESS, 3) != 3) { // Request three unsigned chars
//		return -999;
//	}
	short msb, csb, lsb;
	msb = (short)IIC_Read(OUT_P_MSB);
	csb = (short)IIC_Read(OUT_P_CSB);
	lsb = (short)IIC_Read(OUT_P_LSB);

	//msb = Wire.read(); 0 2
	//csb = Wire.read(); 1 1
	//lsb = Wire.read(); 2 0
//	unsigned char press[3];
//	readBytes(OUT_P_MSB,press,3);
	// The least significant unsigned chars l_altitude and l_temp are 4-bit,
	// fractional values, so you must cast the calulation in (float),
	// shift the value over 4 spots to the right and divide by 16 (since 
	// there are 16 values in 4-bits). 

	float tempcsb = (lsb>>4)/16.0;
//	float tempcsb = (Csb>>4)/16.0;
//	float tempcsb = (press[0] >> 4) /16.0;

	float altitude = (float)( (msb) | csb) + tempcsb;
//	float altitude = (float)( (msb << 8) | csb) + csb;
//	float altitude = (float)( (press[2] << 8) | press[1]) + tempcsb;
...
//	long pressure_whole = (long)press[0]<<16 | (long)press[1]<<8 | (long)press[2];
...
//	float pressure_decimal = (float)press[4]/4.0; //Turn it into fraction
...
/*	Wire.beginTransmission(MPL3115A2_ADDRESS);
	...
	Wire.endTransmission(false); // Send data to I2C dev with option for a repeated start. THIS IS NECESSARY and not supported before Arduino V1.0.1!
	if (Wire.requestFrom(MPL3115A2_ADDRESS, 2) != 2) { // Request two unsigned chars
		return -999;
	}
...
{
  unsigned char tempSetting = IIC_Read(CTRL_REG1); //Read current settings
...
void MPL3115A2::setOversampleRate(unsigned char sampleRate)
{
...
unsigned char MPL3115A2::IIC_Read(unsigned char regAddr)
{
	/*
...
void MPL3115A2::readBytes(unsigned char address,unsigned char * dest,int count)
{
	//read count bytes into dest
	for(int i = 0; i < count;i++)
	{
		dest[i] = IIC_Read(address);
	}
}

void MPL3115A2::IIC_Write(unsigned char regAddr, unsigned char value)
{
	/*
  ...
  */
	  wiringPiI2CWriteReg8(getFd(),regAddr,value);
}

void MPL3115A2::setFd(int a)
{
	fd = a;
	return;
}
