...
public:
  ...
  void setOversampleRate(unsigned char); // Sets the # of samples from 1 to 128. See datasheet.
  void enableEventFlags(); // Sets the fundamental event flags. Required during setup.
  void setFd(int);
  int getFd();
//  void readBytes(unsigned char,unsigned char,unsigned int,unsigned char*);
...
private:
  //Private Functions
  int fd;
  void readBytes(unsigned char,unsigned char*,int);
  void toggleOneShot();
  unsigned char IIC_Read(unsigned char regAddr);
  void IIC_Write(unsigned char regAddr, unsigned char value);
